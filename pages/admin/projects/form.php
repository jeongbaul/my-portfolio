<?php
if (!isset($conn)) include_once $_SERVER['DOCUMENT_ROOT'] . "/lib/db.php";
...
<style>
    ...
    img { max-width:150px; display:block; margin-top:10px; border:1px solid #ccc; border-radius:5px; }
</style>
</head>
<body>
...
<form action="save" method="post" enctype="multipart/form-data">
    ...
    <label for="link">프로젝트 링크(URL)</label>
    <input type="text" name="link" id="link" value="<?= htmlspecialchars($link) ?>" placeholder="예: https://myproject.com">

    <label for="img">이미지 <?= $id && $img ? '(현재 이미지 표시)' : '' ?></label>
    <input type="file" name="img" id="img" accept="image/*">
    <?php if ($id && $img): ?>
        <img src="/uploads/<?= htmlspecialchars($img) ?>" alt="project">
    <?php endif; ?>
